...
class PlaneRecNet(nn.Module):
    def __init__(self, cfg):
        ...
        self.fpn = FPN([src_channels[i] for i in self.fpn_indices],
        ...
        # build the ins head.
        instance_shapes = [cfg.fpn.num_features for _ in range(
            len(cfg.solov2.instance_in_features))]
        self.inst_head = SOLOv2InsHead(cfg, instance_shapes)

        # build the mask head.
        mask_shapes = [cfg.fpn.num_features for _ in range(
            len(cfg.solov2.masks_in_features))]
    ...
    def forward(self, x):
        ...
        with timer.env("fpn"):
            features = self.fpn([features_encoder[i]
                                for i in self.fpn_indices])

        # Instance Branch
        with timer.env("instance head"):
            ins_features = [features[f]
                            for f in range(len(self.instance_in_features))]
        ...
        with timer.env('mask head'):
            mask_features = [features[f]
                             for f in range(len(self.mask_in_features))]
            mask_pred = self.mask_head(mask_features)
            # Depth Decoding
        with timer.env("depth_decoder"):
            depth_pred = self.depth_decoder(
                [features_encoder[i] for i in self.depth_decoder_indices], mask_pred, kernel_pred)
        ...
        with timer.env('Inferencing'):
            if self.training:
                ...
                results = self.inference(
                    mask_pred, cate_pred, kernel_pred, depth_pred, x)
    ...
    def inference(self, pred_masks, pred_cates, pred_kernels, pred_depths, batched_images):
        ...
        for img_idx in range(len(batched_images)):
            ...
            result = self.inference_single_image(
                pred_mask, pred_cate, pred_kernel, pred_depth, ori_size)
            results.append(result)
        return results

    def inference_single_image(self, seg_preds, cate_preds, kernel_preds, depth_pred, ori_size):
        result = {'pred_masks': None, 'pred_boxes': None,
                  'pred_classes': None, 'pred_scores': None, 'pred_depth': None, }

        # depth interpolation
        result['pred_depth'] = F.interpolate(
            depth_pred, size=ori_size, mode='bilinear', align_corners=False).detach()
...
class SOLOv2MaskHead(nn.Module):
    ...
    def forward(self, features):
        ...
        for i in range(1, self.num_levels):
            ...
            feature_add_all_level = feature_add_all_level.clone() + \
                self.convs_all_levels[i](mask_feat)
...
class DepthDecoder_FPN(nn.Module):
    ...
    def __init__(self):
        ...
        self.depth_pred = nn.Sequential(
            nn.ReflectionPad2d(1),
            nn.Conv2d(64, self.num_output_channels,
                      kernel_size=3, stride=1, padding=0),
            nn.Softplus()
        )

        self.conv1x1 = nn.Sequential(
            nn.Conv2d(self.channels_kernels_flatten, 256,
    ...
    def forward(self, feature_maps, seg_preds, kernel_preds):
        ...
        flatten_kernel = torch.cat([kernel_preds[i].permute(0, 2, 3, 1).view(
            B, -1, self.num_kernels) for i in range(num_ins_levels)], dim=1).detach()
        _, N, I = flatten_kernel.shape
        flatten_kernel = flatten_kernel.view(-1, N, I, 1, 1)
        mask_preds = torch.cat([F.conv2d(seg_preds[img_idx].unsqueeze(0).detach(
        ), flatten_kernel[img_idx], stride=1) for img_idx in range(B)], dim=0).sigmoid().detach()
        mask_preds = self.conv1x1(mask_preds)
        mask_preds = F.interpolate(mask_preds, scale_factor=0.25, mode='bilinear',
        ...
        x = self.deconv2(
            torch.cat([self.conv2(self.latlayer2(feats[1])), x], dim=1))
        x = self.deconv3(
            torch.cat([self.conv3(self.latlayer3(feats[2])), x], dim=1))
        x = self.deconv4(
            torch.cat([self.conv4(self.latlayer4(feats[3])), x], dim=1))
        x = self.depth_pred(x)
...
if __name__ == "__main__":
    ...
    def parse_args(argv=None):
        parser = argparse.ArgumentParser(
            description="For PlaneRecNet Debugging and Inference Time Measurement")
        parser.add_argument(
            "--trained_model",
            default=None,
            type=str,
    ...
    frame = torch.from_numpy(cv2.imread(
        "data/example_nyu.jpg", cv2.IMREAD_COLOR)).cuda().float()
