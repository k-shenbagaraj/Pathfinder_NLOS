...
def draw_route(map_size: ndarray, route: ndarray,
    ...
    min_map_size = np.array([min(route[:, 0]), min(route[:, 1])])
    route = route * map_size
    print("Before: ", route.shape)
    route = route.reshape((-1, 1, 2))
    print("After: ", route.shape)
    ...
    # print("len(idxs): ", len(idxs), ax)
    if isinstance(map_size, np.ndarray) and (map_size > 0).all():
        print("using 1")
...
def draw_routes(routes: tuple[ndarray, ndarray], return_mode: str = None):
    ...
    for i, route in enumerate(routes):
        route = route.reshape((-1, 1, 2))
        # print(route.shape, route)
        idxs = np.array(range(route.shape[0]))
        axes[i].plot(route[:, 0, 0], route[:, 0, 1], '--', ms=2)
        ...
        fig.colorbar(line, ax=axes[i], ticks=idxs[::int(len(idxs) / 2)], label='step', fraction=0.05)
